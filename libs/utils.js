module.exports = function (config) {
    ...
    const IUniswapV2Pair = require('@uniswap/v2-core/build/IUniswapV2Pair.json').abi
    const ERC20Abi = require('@uniswap/v2-core/build/IERC20.json').abi

    let contracts = {}
    let decoders = {}

    const web3Url = 'ws://127.0.0.1:8545' // wss://mainnet.infura.io/ws/v3/e4686b67f9f8418585ae4a9c6c04857f process.env.ETH_NODE_URL || config.DEFAULT_NODE_URL
    ...
    const insertPairStatement = db.prepare('INSERT OR IGNORE INTO pairs \
    ...
    function savePair(pairData) {
        try {
            ...
            if (res.changes === 1) {
                console.info(`New pair saved ${pairData.quote} ${pairData.address} ${pairData.token0_address} ${pairData.token1_address}`)
